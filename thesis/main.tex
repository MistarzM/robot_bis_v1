...
\begin{itemize}
    ...
    Stanowi centrum decyzyjne i percepcyjne systemu. Rolę tę pełni zewnętrzny komputer (laptop) wyposażony w środowisko zdolne do szybkiej inferencji modelu detekcji obiektów. Jednostka ta odbiera surowy strumień wideo z robota, poddaje go analizie w poszukiwaniu docelowych obiektów, a następnie – na podstawie uzyskanych współrzędnych z ramki obrazu – oblicza wymagane komendy ruchowe i docelowe kąty dla poszczególnych stawów ramienia manipulacyjnego. Obliczone pakiety danych sterujących są niezwłocznie odsyłane z powrotem do robota.
    
    \item \textbf{Węzeł sterujący (ang. \textit{Controller Node}) -- Jednostka pokładowa:}
    Zadanie to realizowane jest przez mikrokomputer Raspberry Pi 5 znajdujący się fizycznie na platformie jezdnej. Pełni on rolę inteligentnego interfejsu (mostu) pomiędzy światem fizycznym a stacją operatorską. Do jego głównych zadań należy nieprzerwana akwizycja obrazu z kamery USB i strumieniowanie go przez sieć bezprzewodową Wi-Fi. Równolegle mikrokomputer ten nasłuchuje komend napływających z węzła obliczeniowego. Po odebraniu pożądanych kątów dla serwomechanizmów, Raspberry Pi przekazuje je interfejsem szeregowym do sterownika ESP32 (Servo Driver), który zajmuje się już bezpośrednim wysterowaniem napędów.
\end{itemize}

Taki ścisły podział kompetencji niesie za sobą wymierne korzyści inżynierskie. Odciąża procesor jednostki pokładowej, pozwalając mu skupić się wyłącznie na utrzymaniu bezbłędnej komunikacji i płynności strumienia wideo. Jednocześnie zapewnia elastyczność – stację operatorską można w dowolnym momencie zmodernizować w celu uzyskania większej liczby klatek na sekundę (FPS) bez konieczności ingerencji w sprzętową konstrukcję samego robota.

\section{Topologia sieci i komunikacja bezprzewodowa}
\section{Zapewnienie bezpieczeństwa logicznego}


% =========================================================================
% ROZDZIAŁ 6
% =========================================================================
\chapter{Implementacja oprogramowania wbudowanego}
\section{Środowisko uruchomieniowe i procedury startowe}
\section{Obsługa silników i odczyt telemetrii}
\section{Implementacja kinematyki analitycznej ramienia}
\section{Procedury kalibracji osi i bezpieczeństwo ruchu}
\section{Przesył strumienia wideo z kamery pokładowej}


% =========================================================================
% ROZDZIAŁ 7
% =========================================================================
\chapter{Implementacja modułu uczenia maszynowego}
\section{Przygotowanie i konfiguracja modelu detekcji}
\section{Przetwarzanie obrazu w czasie rzeczywistym i klasyfikacja obiektów}


% Poniższa komenda wymusza przeniesienie Rozdziału 8 na nową stronę W SPISIE TREŚCI
...
% ROZDZIAŁ 8
% =========================================================================
\chapter{Stworzenie aplikacji klienckiej}
\section{Projekt graficznego interfejsu użytkownika}
\section{Integracja bezprzewodowego kontrolera i mapowanie przycisków}
\section{Tryby sterowania}
\section{Wizualizacja telemetrii, wideo i logowanie systemu}
...
\section{Testy kinematyki ramienia i precyzji chwytania obiektów}
\section{Ewaluacja skuteczności algorytmów rozpoznawania obrazu}
\section{Testy responsywności systemu i mechanizmów bezpieczeństwa}
